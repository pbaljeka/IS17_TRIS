...
def get_missing_nodes(missingnodefile):
    nodelist=[]
    with open(missingnodefile, 'r') as f:
	for line in f:
...
def make_binary_tris_feats(tris_file, indir, outdir):
    ...
    with open(full_path +'.tris', 'r') as f:
		...
		yes_node=line.strip().split()[1]
		no_node=line.strip().split()[2]
		if yes_node not in missing_nodes and no_node not in missing_nodes:
...
if __name__=="__main__":
    option=sys.argv[1]
    utils_path="/home/pbaljeka/TRIS_Exps3/utils-clb5/"
    if option=="uttwise":
	indir="coeffs5"
	outdir="nn_tris5"
