...
def calc_acoustic_stats(data_path, utils_dir,  chunksize=100, feature_simension=52):
    filelist= utils_dir + '/train_list'
    merlin_io=binary_io.BinaryIOCollection()
    for ext in ['ymean', 'ystd', 'nmean', 'nstd']:
...
def calc_acoustic_stats_root(data_path, utils_dir,  chunksize=100, feature_simension=52):
    filelist= utils_dir + '/train_list'
    merlin_io=binary_io.BinaryIOCollection()
    for ext in ['mean', 'std']:
        print('Calculating stats for : ', ext)
...
def do_lab_normalization(data_dir, utils_dir, n_input_dim=1488):
    ...
    train_list=utils_dir + '/train_list'
    val_list=utils_dir + '/val_list'
    test_list=utils_dir +'/test_list'
    ...
    binary_label_train_list   = prepare_file_path_list(train_list, binary_label_dir, '.tris')
...
def do_lab_normalization_for_prediction(data_dir, utils_dir, n_input_dim=1488):
    logger=logging.getLogger("label_normalization")
    logger.info('Doing input feature normaization')
    train_list=utils_dir + '/train_list'
...
def do_acoustic_normalisation(data_dir, utils_dir, n_input_dim=52):
    ...
    merlin_io=binary_io.BinaryIOCollection()
    for stat in ['ymean', 'ystd', 'nmean', 'nstd']:
...
def do_acoustic_normalisation_root(data_dir, utils_dir, n_input_dim=52):
    logger=logging.getLogger("acoustic_normalization")
    logger.info('normalising acoustic (output) features using MVN')
    all_list=utils_dir +'/filelist'
...
def do_acoustic_denormalisation(data_dir, utils_dir, n_input_dim=52):
    logger=logging.getLogger("acoustic_denormalization")
    logger.info('normalising acoustic (output) features using MVN')
    all_list=utils_dir +'/all_list'
    in_data_dir= data_dir + '/norm_nn_tris/'
    out_data_dir=data_dir + '/predicted_nn_tris/'
    if not os.path.exists(out_data_dir):
        os.makedirs(out_data_dir)
...
def do_acoustic_denormalisation_trees(data_dir, utils_dir, model_name, n_input_dim=52):
    logger=logging.getLogger("acoustic_denormalization")
    logger.info('normalising acoustic (output) features using MVN')
    all_list=utils_dir +'/predict_trees_filelist'
...
def do_acoustic_denormalisation_trees_root(data_dir, utils_dir, model_name, n_input_dim=52):
    logger=logging.getLogger("acoustic_denormalization")
    logger.info('normalising acoustic (output) features using MVN')
    all_list=utils_dir +'/sen_list'
    in_data_dir= data_dir + '/predicted_norm_nn_tris_trees/' + model_name + '/'
    out_data_dir=data_dir + '/predicted_norm_nn_tris_trees/un_norm_' + model_name + '/'
    if not os.path.exists(out_data_dir):
        os.makedirs(out_data_dir)
...
if __name__=="__main__":
    ...
    #do_lab_normalization_for_prediction(data_dir, utils_dir, int(1488))
    #calc_acoustic_stats_root(data_dir + '/nn_tris/', utils_dir)
    #do_acoustic_normalisation_root(data_dir , utils_dir)
   #do_acoustic_denormalisation(data_dir , utils_dir)
    do_acoustic_denormalisation_trees_root(data_dir , utils_dir, model_name)
